...
export default function PaginaCursos() {
  ...
  const [cursosFiltrados, setCursosFiltrados] = useState<CursoItem[]>([]);
  ...
  // Carrega usuário atual e cursos do localStorage
  useEffect(() => {
    ...
    if (cursosSalvos) {
      try {
        const parsedCursos: CursoItem[] = JSON.parse(cursosSalvos);
        setCursos(parsedCursos);
        setCursosFiltrados(parsedCursos);
      } catch (error) {
  ...
  // Sempre que os cursos ou termo de pesquisa mudarem, atualiza localStorage e filtrados
  useEffect(() => {
    if (usuarioAtual) {
  ...
  const validarCurso = (curso: Omit<CursoItem, "id">) => {
    ...
    if (!curso.materialApoio || !curso.materialApoio.nome || !curso.materialApoio.url) {
      alert("Material de Apoio é obrigatório.");
      return false;
    }
    if (
      curso.categoria === CATEGORIA_AOVIVO &&
      (!curso.zoomLink || !curso.zoomDataHora || (curso.zoomDuracao !== undefined && curso.zoomDuracao <= 0))
    ) {
  ...
  const editarCursoHandle = (curso: CursoItem) => {
    ...
    setEdicaoCurso({
      ...
      materialApoio: curso.materialApoio || { nome: "", url: "" },
  ...
  const handleFileUpload = (
    ...
    if (!file) {
      if (fieldName === "materialApoio") {
        setState((prev) => ({ ...prev, materialApoio: { nome: "", url: "" } }));
  ...
  const renderFormFields = (
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
      ...
      <div className="md:col-span-2 space-y-4">
        <div>
          <label
            htmlFor={isEditing ? "edit-titulo" : "novo-titulo"}
        ...
        <div>
          <label
            htmlFor={isEditing ? "edit-capa" : "novo-capa"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
        ...
        <div>
          <label
            htmlFor={isEditing ? "edit-conteudo" : "novo-conteudo"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
        ...
        <div>
          <label
            htmlFor={isEditing ? "edit-desc-curta" : "novo-desc-curta"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
      ...
      <div className="space-y-4">
        <div>
          <label
            htmlFor={isEditing ? "edit-categoria" : "novo-categoria"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Categoria*
          </label>
          <select
            ...
            onChange={(e) => {
              ...
              setState({
                ...
                zoomDataHora:
                  newCategoria === CATEGORIA_BASICO ? "" : state.zoomDataHora,
                zoomDuracao:
                  newCategoria === CATEGORIA_BASICO ? 0 : state.zoomDuracao,
        ...
        <div>
          <label
            htmlFor={isEditing ? "edit-docente" : "novo-docente"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Registrar Docente
          </label>
        ...
        <div>
          <label
            htmlFor={isEditing ? "edit-material" : "novo-material"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Material de Apoio*
          </label>
          <input
            ...
            className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#E8E8D7] file:text-[#281719] hover:file:bg-[#D8D8C7]"
          />
          {state.materialApoio?.nome && (
            <p className="text-xs text-gray-500 mt-1">
              Atual: {state.materialApoio.nome}
            </p>
          )}
        </div>
        ...
        <div>
          <label
            htmlFor={isEditing ? "edit-tags" : "novo-tags"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Tags
          </label>
        ...
        <div>
          <label
            htmlFor={isEditing ? "edit-dataInicio" : "novo-dataInicio"}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Data de início
          </label>
        ...
        {state.categoria === CATEGORIA_AOVIVO && (
          <div className="p-3 border-t mt-4 space-y-3 bg-gray-50 rounded-md">
            <h3 className="text-sm font-medium text-gray-700">
              Detalhes da Reunião Zoom (Curso ao vivo)
            </h3>
            <div>
              <label
                htmlFor={isEditing ? "edit-zoomLink" : "novo-zoomLink"}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label
                  htmlFor={isEditing ? "edit-zoomDataHora" : "novo-zoomDataHora"}
                  className="block text-xs font-medium text-gray-600 mb-1"
                >
                  Data e Hora*
                </label>
              ...
              <div>
                <label
                  htmlFor={isEditing ? "edit-zoomDuracao" : "novo-zoomDuracao"}
                  className="block text-xs font-medium text-gray-600 mb-1"
                >
                  Duração (min)*
                </label>
                <input
                  ...
                  onChange={(e) =>
                    setState({ ...state, zoomDuracao: parseInt(e.target.value, 10) || 0 })
                  }
                  className="w-full p-2 border border-gray-300 rounded-md shadow-sm text-sm focus:ring-[#9BB61B] focus:border-[#9BB61B]"
                  min="1"
                />
            ...
            <div>
              <label
                htmlFor={isEditing ? "edit-zoomId" : "novo-zoomId"}
                className="block text-xs font-medium text-gray-600 mb-1"
              >
            ...
            <div>
              <label
                htmlFor={isEditing ? "edit-zoomSenha" : "novo-zoomSenha"}
                className="block text-xs font-medium text-gray-600 mb-1"
              >
  ...
  const handleAcessarCurso = (cursoId: number) => {
    // Navegar para página de detalhes do curso (exemplo: /cursos/[id])
  ...
  return (
    <div className="p-4 md:p-6 space-y-8 bg-[#FFFCE5] min-h-screen">
      ...
      {/* Formulário de Criação/Edição (visível apenas para admin) */}
      {usuarioAtual?.email === "admin@admin.com" && (
        <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl space-y-6 border border-gray-200">
          <h2 className="text-2xl font-semibold text-[#281719] border-b pb-3">
            {cursoEditando !== null
              ? `Editando Curso: ${edicaoCurso.titulo}`
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cursosFiltrados.length === 0 && (
          <p className="text-gray-600 md:col-span-full">
            Nenhum curso ou webinar encontrado.
          </p>
        )}
            ...
            <div className="p-5 flex flex-col flex-grow space-y-2">
              ...
              <div className="text-xs text-gray-500 space-y-0.5">
                ...
                {curso.dataInicio && (
                  <p>
                    Início:{" "}
                    {new Date(curso.dataInicio + "T00:00:00").toLocaleDateString(
                      "pt-BR"
                    )}
                  </p>
                )}
              ...
              {curso.categoria === CATEGORIA_AOVIVO && curso.zoomLink && (
                <div className="mt-2 pt-2 border-t border-gray-100 space-y-1">
                  <p className="text-xs text-gray-500">
                    ...
                    {curso.zoomDataHora
                      ? new Date(curso.zoomDataHora).toLocaleString("pt-BR", {
                          day: "2-digit",
                          month: "short",
                          hour: "2-digit",
                          minute: "2-digit",
                      ...
                      : "Não definido"}
                    {curso.zoomDuracao ? ` (${curso.zoomDuracao} min)` : ""}
                  </p>
                </div>
              )}
              ...
              <div className="mt-auto pt-3">
                {(usuarioAtual?.email === "admin@admin.com") ||
                curso.categoria === CATEGORIA_BASICO ||
                (curso.categoria === CATEGORIA_AOVIVO && !curso.zoomLink) ? (
                  <button
